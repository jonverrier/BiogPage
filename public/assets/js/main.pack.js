  ...
  window.addEventListener('load', () => {
   ...
   function onTimer(params) {
      ...
      if (element) {
         ...
         nextIndex = (params.index + 1) % (params.images.length);
         params.index = nextIndex;

         // Set the next background image 
         element.style.backgroundImage = params.images[nextIndex];
      }
      return 0;
   }

   function cycleBackground(elementIn, imagesIn) {
      var params = {
         elementId: elementIn,
         images: imagesIn,
         index: 0
   ...
   window.addEventListener('load', () => {
      cycleBackground('about-pics-s', [makeImageUrl("/assets/img/sariwd-solo-r-s.jpg"),
